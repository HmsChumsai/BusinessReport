...
local function process()
  ...
  if(orderVolumeRisk ~= nil) then				
    ...
    table.insert (depositItem, {'unreal_pl_expected', easygetter.EvenAmountToDouble(orderVolumeRisk:getTotalFutureUPLGross())})
    ...
    -- The value of Call Force Flag Expected is same as Call Force Flag Port
  ...
  -- What we get here is a table/list of order numbers
          ...
          if op:getTransactionType() == "Match" then
            ...
            elseif(oplegs[1]:getContract():getContractCode() == orderLeg:getContract():getContractCode()) then
              ...
              if (opleg:hasTransaction()) then
                ...
                local cdate = ta:getBusinessDate()
                table.insert (orderItem, {'cdate', cdate:toString("%d/%m/%Y")})
                cont_size = matched_value
