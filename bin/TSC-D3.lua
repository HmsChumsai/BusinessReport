...
local orderVolLimit = easygetter.GetFirstActiveOVL(depositId)
  ...
  if(orderVolumeRisk ~= nil) then
    ...
    --table.insert (depositItem, {'unreal_pl_expected', easygetter.EvenAmountToDouble(orderVolumeRisk:getTotalFutureUPLGross())+easygetter.EvenAmountToDouble(orderVolumeRisk:getTotalOptionUPLGross())}) --------
  ...
  for _,no in pairs( orders ) do
    ...
    if (orderHandlingType == 'TradingOrder' or orderHandlingType == 'BlockTrade') then
      ...
      for i = 1, order:getNumberLegs() do
        ...
        local orderStatus = fo.getCustomOrderStatus(order:getOrderId())
